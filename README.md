...
- 临时限流、上游过载或网络错误进入冷却，Agent 保持连接和心跳。首次约 60 秒，反复失败按 120、240 秒递增，普通退避最高约 5 分钟并加抖动；显式 Retry-After 最多尊重 10 分钟。
- 未识别的执行错误连续 5 次也进入恢复流程，不再永久停工。冷却到期先同步解除 Agent 的本地失败锁，再只派发一个队列任务探测；成功后恢复正常消费，失败则继续冷却。恢复状态、截止时间及任务重试次数保存在数据库，重连/重启后继续。
- 按现有部署的共享模型容量策略，恢复期只允许一个队列探测，可能暂时影响使用不同模型的其他 Agent；direct/broadcast 不走此自动重试策略。已有在途任务不会为探测而被中止。
- 普通失败每任务最多重试 3 次，临时错误最多重试 8 次（不含首次执行）。任务耗尽预算后保留失败记录，Agent 仍可探测后续任务；不是无界重试同一个坏任务。401/403、无效 API Key、额度/计费错误停止自动尝试，需修复配置。
- Agent 卡片显示「冷却中 / 恢复探测 / 需处理」、原因和下次尝试时间。在卡片「···」菜单中选择「立即重试（单任务探测）」或「恢复接单」。操作使用原有认证接口 `POST /api/agents/:employeeId/resume-queue`。手动暂停始终优先，不会被自动恢复解除。
- 清空/取消不会被恢复计时器复活；清空任务不删除 Agent 配置和人工暂停状态，也不等价于压缩 SQLite 文件或清理主机日志。

离线 Agent 无法仅靠中心服务重新启动：目前没有远程主机管理协议。对于使用内置 daemon 启动的 Agent，请在 **该 Agent 所在机器、相同运行用户和环境** 下执行 `ai-teams-agent status`，必要时执行 `ai-teams-agent restart`；重启会中断当前子进程，先核对在途任务。由 systemd / Docker 管理的实例应使用对应服务管理方式，不要另起第二个同 ID 进程。Web 的「恢复接单」不会重启进程，也不能修复失效凭据。

建议 Server 与 Agent 一起升级。新 Agent 在重连握手中声明待补发的完成回执，避免服务器过早启动新 attempt；旧 Agent 不具备此保护。网络分区时远端旧进程仍可能执行，本系统不是 exactly-once 执行器：涉及外部写入的任务仍需业务幂等，不能把忽略旧回执当作已经停止旧进程。
...
Web 控制台 → **任务日志** → **清空所有任务**，确认后清理服务端全部任务，不受当前筛选或分页限制。任务日志也提供 **等待中** 筛选，对应 `status=queued`（包括共享队列和等待执行的定向任务）。

手机端可从顶部 **任务列表** 进入，操作后通过 **返回对话** 回到聊天页面。
...
- 删除所有状态的任务、输出日志、webhook 配置，以及 Mission、子任务关联、事件和审批历史。
- 清空期间暂停派发并拒绝新的 API 写入（HTTP 503）；等待已有写入和 Mission 编排结束后，在数据库事务内删除，随后清理内存队列、重试和超时计时器，并实时通知所有在线控制台。
- 向在线 Agent 发送终止请求；在收到终止/结束回执前，不向该 Agent 派发新任务。离线 Agent 的进程无法立即确认停止，重连时会请求终止已不存在的旧任务。建议 Server、Web 与 Agent 一起更新。
- 保留员工、Agent 注册/令牌、暂停状态和定时任务配置。启用中的定时任务在清空后仍可能产生新任务；若希望队列持续为空，请先在「定时任务」页面停用相应计划，并停止外部任务提交方。
- 返回 `deleted.tasks`、`deleted.missions`、`cancellationRequests`、`offlineTasks` 和 `enabledSchedules`，分别表示删除记录数、终止请求数、无法立即终止的离线任务数及仍启用的定时任务数。

删除不可撤销。清空不会删除 Agent 本机的 Claude 会话/文件或应用日志；数据库释放的页可供复用，但不保证数据库文件立刻缩小，不在在线请求中执行 `VACUUM`。

开发验证：`pnpm build` 后可运行 `node scripts/test-task-cleanup.mjs`。该脚本需要 Playwright 和 Chromium（可用 `PLAYWRIGHT_MODULE_PATH` 指向已有安装），仅启动本机随机端口及临时 SQLite 数据库，验证筛选、确认取消、多页面同步、迟到响应、刷新和手机入口，不连接真实 Agent 或外部数据库。
